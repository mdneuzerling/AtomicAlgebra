...
class AtomicAlgebra:
    ...
    def __init__(self, atom_table, converse = None):
        ...
        if type(converse) is dict:
            ...
            self.atom_converses = converse
        # ... or as a list of tuples.
        else:
            self.converse_pairs = converse
            self.atom_converses = self.converse_pairs_to_dict(converse)
    ...
    def preserves_composition(self, algebra2, atom_map):
        preserves_composition = True
        for x, y in product(self.atoms, repeat = 2):
            if AtomicAlgebra.atom_function(atom_map, self.compose(x, y)) != algebra2.compose(AtomicAlgebra.atom_function(atom_map, x), AtomicAlgebra.atom_function(atom_map, y)):
    ...
    def is_isomorphic(self, algebra2, return_isomorphism = False):
        ...
        if self.n_atoms != algebra2.n_atoms: return False
        ...
        if len(self.symmetric_atoms) != len(algebra2.symmetric_atoms):
            return False
        ...
        for perm in permutations(algebra2.symmetric_atoms):
        ...
        for perm1 in list(product(*[[x,x[::-1]] for x in algebra2.non_symmetric_pairs])):
        ...
        algebra2_identity_permutations = [p for p in permutations(list(algebra2.identity))]
        possible_identity_maps = [dict((list(self.identity)[i], y[i]) 
            for i in range(len(self.identity))) 
            for y in algebra2_identity_permutations]
        ...
        for possible_isomorphism in possible_isomorphisms:
            if self.preserves_composition(algebra2, possible_isomorphism):
                is_isomorphic = True
    ...
    def converse(self, x):
        x = self.make_set(x)
        return set([self.atom_converses[atom] for atom in x])
