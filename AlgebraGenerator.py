...
# Suppose that the identity element consists of n_atoms, with n_atoms > 1.
...
def fragment_identity(n_atoms, n_fragments):
    if n_fragments == 1:
        ...
        atoms = [chr(i + 97) for i in range(n_atoms)]
        return([(atom, atom, atom) for atom in atoms[:n_fragments]] + 
            [(atom1, atom2, 0) for atom1, atom2 in product(atoms[:n_fragments], repeat=2) if atom1 != atom2])
...
def gen_cycle_partition(converse, n_atoms):
    atoms = [chr(i + 97) for i in range(n_atoms)]
    parts = []
    for triple in product(atoms, repeat = 3):
        cycle = gen_cycle(triple, converse)
...
def generate_algebras_from_fixed_entries(entries_to_fix, converse, n_atoms, fix_identity_size = False):
    ...
    cycles = gen_cycle_partition(converse, n_atoms)
    ...
    # non-symmetric atoms.
def generate_algebras_from_identity_and_converse(n_atoms, identity_size, converse):
...
def generate_algebras(n_atoms):
    ...
    for identity_size in range(1, n_atoms + 1):
        for converse in converses_respecting_identity(n_atoms, identity_size):
            algebras += generate_algebras_from_identity_and_converse(n_atoms, identity_size, converse)
    return(algebras)
